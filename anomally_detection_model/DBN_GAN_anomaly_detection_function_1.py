...
class RBM_model(object):
    def __init__(self, input_value=None, n_visible=visible, n_hidden=hidden, w=None, h_bias=None, v_bias=None):
    ...
    def visible_to_hidden(self, n_visible) -> object:
        n_visible = tf.cast(tf.convert_to_tensor(n_visible), dtype=tf.float32)
        return tf.keras.activations.sigmoid(tf.matmul(n_visible, self.w) + self.h_bias)

    def hidden_to_visible(self, n_hidden) -> object:
        n_hidden = tf.cast(tf.convert_to_tensor(n_hidden), dtype=tf.float32)
        return tf.keras.activations.sigmoid(tf.matmul(n_hidden, tf.transpose(self.w)) + self.v_bias)
    ...
    def train(self):
        for epoch in range(training_epochs):
            avg_cost = 0
            cost, output = self.get_reconstruction_cost()
...
class Neural_network_model(object):
    def __init__(self, input_value):
        ...
        self.loss_func = tf.keras.losses.mape
    ...
    def train_step(self):
        with tf.GradientTape() as tape:
            ...
            train_loss = self.loss_func(self.trains, predictions)
