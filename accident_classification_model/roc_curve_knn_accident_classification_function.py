...
def knn_classification(y_predict, y_true) -> object:
    TP = 0
    FN = 0
    FP = 0
    TN = 0
...
def knn_model(x_data_set, y_data_set, n_neighbors, weights, algorithm) -> object:
...
def knn_model_function(result_directory, x_data):
    y_data_set = np.hstack((np.full(5000, 1), np.zeros(20000)))

    x_data_set = np.vstack((x_data[0], x_data[1], x_data[2], x_data[3], x_data[4]))
    y_predict, y_true = knn_model(x_data_set, y_data_set, 1, 'uniform', 'auto')
    is_power_decreasing_result, is_power_decreasing_roc = knn_classification(y_predict, y_true)

    x_data_set = np.vstack((x_data[1], x_data[0], x_data[2], x_data[3], x_data[4]))
    y_predict, y_true = knn_model(x_data_set, y_data_set, 9, 'uniform', 'auto')
    is_prz_space_leak_result, is_prz_space_leak_roc = knn_classification(y_predict, y_true)

    x_data_set = np.vstack((x_data[2], x_data[0], x_data[1], x_data[3], x_data[4]))
    y_predict, y_true = knn_model(x_data_set, y_data_set, 1, 'uniform', 'auto')
    is_rcs_loca_result, is_rcs_loca_roc = knn_classification(y_predict, y_true)

    x_data_set = np.vstack((x_data[3], x_data[0], x_data[1], x_data[2], x_data[4]))
    y_predict, y_true = knn_model(x_data_set, y_data_set, 8, 'uniform', 'auto')
    is_sg_2nd_side_leak_result, is_sg_2nd_side_leak_roc = knn_classification(y_predict, y_true)

    x_data_set = np.vstack((x_data[4], x_data[0], x_data[1], x_data[2], x_data[3]))
    y_predict, y_true = knn_model(x_data_set, y_data_set, 1, 'uniform', 'auto')
    is_sgtr_result, is_sgtr_roc = knn_classification(y_predict, y_true)

    result_path = result_directory + "knn_roc_result.txt"
